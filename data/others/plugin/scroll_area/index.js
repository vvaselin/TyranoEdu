(function () {
  ...
  new_tag['scroll_area_vertical'] = {
    ...
    start: function (pm) {
      ...
      var scroll_view = $("<div />").css({
        ...
        "pointer-events": "auto"
      }).addClass("scroll_vertical_scrollable").attr('id', pm.id + "_view");

      // マウスホイール（縦スクロールは通常のホイール動作をそのまま使用）
      // 特別な処理は不要だが、明示的に設定する場合は以下のようにする
      scroll_view.on('wheel', function(e) {
        var delta = e.originalEvent.deltaY ? e.originalEvent.deltaY : e.originalEvent.wheelDelta * -1;
      ...
      var btn_up = $("<div class='scroll_vertical_arrow_btn up'><span>▲</span></div>");
      var btn_down = $("<div class='scroll_vertical_arrow_btn down'><span>▼</span></div>");
      ...
      var startScrolling = function(direction) {
        if (scrollInterval) return; 
        scrollInterval = setInterval(function() {
          var current = scroll_view.scrollTop();
          scroll_view.scrollTop(current + (direction * scrollSpeed));
        }, 16); // 約60fpsで滑らかに移動
      };
      ...
      // ボタン押し下げイベント
      btn_up.on('mousedown touchstart', function(e) { startScrolling(-1); e.preventDefault(); });
      btn_down.on('mousedown touchstart', function(e) { startScrolling(1); e.preventDefault(); });

      // 停止条件（ボタンから離れた、指を離した、ボタン外へ出た）
      btn_up.on('mouseup mouseleave touchend', stopScrolling);
      btn_down.on('mouseup mouseleave touchend', stopScrolling);
      ...
      var inner_div = $("<div />").css({
        "width": pm.width + "px",
        "height": pm.contents_h + "px",  // 縦方向の高さ
        "position": "relative"
      }).attr('id', pm.id + "_inner");

      scroll_view.append(inner_div);
      main_wrapper.append(scroll_view);
      main_wrapper.append(btn_up);
      main_wrapper.append(btn_down);
      target_layer.append(main_wrapper);
  ...
  new_tag['scroll_area_vertical_in'] = {
    vital: ["id", "name"],
    start: function (pm) {
      var ary = pm.name.split(',');
      var $inner = $('#' + pm.id + "_inner");
  ...
  new_tag['scroll_area_vertical_del'] = {
    vital: ["id"],
    start: function (pm) {
      // ウィンドウに貼ったイベントも解除
      $(window).off('.' + pm.id);
