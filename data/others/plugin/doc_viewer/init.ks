...
[macro name="show_doc_button"]
    ...
    [iscript]
        ...
        function createMenuItem(item) {
            var li = $('<li class="doc-menu-item" data-file="' + item.file + '">' + item.title + '</li>');
        ...
        $.ajax({
            ...
            success: function(data) {
                createMenu(data);
                var initialFile = mp.file || "top.md";
        ...
        TYRANO.kag.stat.f.loadDocMarkdown = function(file) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    // ハイライト更新
                    var normalizedFile = file.replace("./data/others/plugin/doc_viewer/docs/", "");
                    $(".doc-menu-item").removeClass("current");
                    $(".doc-menu-item[data-file='" + normalizedFile + "']").addClass("current");
...
[macro name="show_doc_full"]
    ...
    [iscript]
        ...
        // ミドルクリックでオートスクロール
        contentArea.on("mousedown", function(e) {
            ...
            $scrollTarget = $(this);
            $scrollTarget.css("cursor", "ns-resize");

            function doScroll() {
                if (!isAutoScrolling) return;
                var el = $scrollTarget[0];
                el.scrollTop += autoScrollSpeed;
                autoScrollRAF = requestAnimationFrame(doScroll);
            }
            doScroll();
        });
        ...
        $(document).on("mousemove.autoscroll", function(e) {
            if (!isAutoScrolling) return;
            var diff = e.clientY - autoScrollStartY;
            // 中央付近はデッドゾーン
            if (Math.abs(diff) < 10) {
                autoScrollSpeed = 0;
            } else {
                autoScrollSpeed = diff * 0.1; // 速度係数（調整可）
            }
        });

        $(document).on("mouseup.autoscroll click.autoscroll", function(e) {
            if (!isAutoScrolling) return;
            isAutoScrolling = false;
            autoScrollSpeed = 0;
            cancelAnimationFrame(autoScrollRAF);
            if ($scrollTarget) $scrollTarget.css("cursor", "");
        });
